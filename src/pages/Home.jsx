    ...
    <>
          ...
          <div className="grid w-full items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7 }} className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
                <Link to="/vuelos" className="btn-primary">{t('common.exploraDestinos')} <ArrowGo className="h-4 w-4" /></Link>
                <a href={contact.whatsappHref} target="_blank" rel="noreferrer" className="btn inline-flex border border-white/40 text-white hover:bg-white/10">{t('common.hablaAgente')}</a>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.9 }} className="mt-10 flex items-center gap-4">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-white/70">{t('home.clientes')}</p>
                </div>
              </motion.div>
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.5, ease }} className="lg:justify-self-end lg:pl-6">
              <p className="mb-3 hidden font-display text-lg font-bold text-white lg:block">{t('home.widgetLabel')}<span className="text-cyan-300">.</span></p>
              <HeroSearch />
            </motion.div>
          </div>
        </div>
      ...
      <section className="border-b border-line bg-white">
        <div className="container-x grid grid-cols-2 gap-px lg:grid-cols-4">
          {t('home.guarantees').map((g, i) => {
            const Icon = guaranteeIcons[i]
            return (
              <Reveal key={g.title} delay={i * 0.08} className="flex items-start gap-4 py-7 pr-4 sm:px-6">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-cyan-50 text-cyan-600"><Icon className="h-5 w-5" /></span>
                <div>
      ...
      <section className="container-x py-20 sm:py-28">
        <SectionHeading kicker={t('home.servicesKicker')} title={t('home.servicesTitle')} intro={t('home.servicesIntro')} />
      ...
      <section className="relative overflow-hidden border-y border-line bg-mist py-20 sm:py-28">
        <div className="container-x relative">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <SectionHeading kicker={t('home.destinosKicker')} title={t('home.destinosTitle')} intro={t('home.destinosIntro')} />
            <Link to="/vuelos" className="hidden items-center gap-1 text-sm font-bold text-cyan-600 hover:text-cyan-700 sm:inline-flex">{t('common.verTodos')} <ArrowUp className="h-4 w-4" /></Link>
          </div>
      ...
      <section className="overflow-hidden border-t border-line bg-mist py-20 sm:py-28">
        <div className="container-x">
          <SectionHeading align="center" kicker={t('home.reviewsKicker')} title={t('home.reviewsTitle')} intro={t('home.reviewsIntro')} className="mb-12" />
        </div>
        <ReviewMarquee />
      </section>
