...
export default function BuscarVuelos() {
  ...
  const subtitle = payload
    ? [
        payload.flightType === '1' ? t('buscarVuelos.idaVuelta') : t('buscarVuelos.soloIda'),
        fmtDate(payload.startDt),
        payload.endDt ? `→ ${fmtDate(payload.endDt)}` : null,
        `${payload.adults} ${payload.adults > 1 ? t('buscarVuelos.adultos') : t('buscarVuelos.adulto')}`,
        payload.children > 0 ? `${payload.children} ${payload.children > 1 ? t('buscarVuelos.ninos') : t('buscarVuelos.nino')}` : null,
        payload.infants > 0 ? `${payload.infants} ${payload.infants > 1 ? t('buscarVuelos.bebes') : t('buscarVuelos.bebe')}` : null,
      ]
        .filter(Boolean)
        .join(' · ')
    : ''
      ...
      <section className="flex min-h-[70vh] items-center bg-mist pt-24">
        ...
        <div className="container-x text-center">
          <div className="mx-auto max-w-md rounded-3xl border border-line bg-white p-10 shadow-card">
            ...
            <h1 className="mt-4 font-display text-2xl font-bold text-ink">{t('buscarVuelos.invalidTitle')}</h1>
            <p className="mt-2 text-sm text-ink-700">{t('buscarVuelos.invalidText')}</p>
            <Link to="/" className="btn-primary mt-6"><ArrowBack className="h-4 w-4" /> {t('common.volverInicio')}</Link>
          </div>
        </div>
      </section>
    )
    ...
    <>
      ...
      <section className="relative overflow-hidden border-b border-line bg-mist pt-28">
        ...
        <div className="container-x relative pb-10">
          <nav className="mb-4 flex items-center gap-1.5 text-xs font-semibold text-ink-500">
            <Link to="/" className="hover:text-cyan-600">{t('nav.inicio')}</Link>
            <ChevronRight className="h-3.5 w-3.5" />
            <span className="text-cyan-600">{t('buscarVuelos.breadcrumb')}</span>
          </nav>
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <span className="kicker mb-2"><Plane className="h-3.5 w-3.5" /> {t('buscarVuelos.kicker')}</span>
            ...
            <div className="flex flex-wrap gap-2.5">
              <Link to="/vuelos" className="btn-ghost"><ArrowBack className="h-4 w-4" /> {t('common.nuevaBusqueda')}</Link>
      ...
      <section className="bg-white py-8 sm:py-12">
        <div className="container-x">
          ...
          >
            <iframe
              title={t('buscarVuelos.breadcrumb')}
              name={IFRAME_NAME}
